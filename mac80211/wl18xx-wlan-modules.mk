...
WL18XX_MODULES: MLIST = $(strip \
	$(if $(shell grep ^CONFIG_WL12XX=m$$ $<),wl12xx,) \
	$(if $(shell grep ^CONFIG_WL18XX=m$$ $<),wl18xx,))

# A couple of useful macros
commify = $(shell echo $(1) | tr \  ,)
slashdup = $(foreach wd,$(1),$(wd)/$(wd))

WL18XX_MODULES: $(KERNEL_OUT)/.config
	...
	$(if $(ARM_EABI_TOOLCHAIN),$(ARM_EABI_TOOLCHAIN)/arm-eabi-strip,$(KERNEL_TOOLCHAIN_PATH)strip) \
		--strip-unneeded $(KERNEL_MODULES_OUT)/{compat,{cfg,mac}80211,wlcore{,_sdio},$(MLIST)}.ko
